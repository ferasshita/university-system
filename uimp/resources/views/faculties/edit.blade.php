...
@section('content')
    <div class="card">
        <div class="card-header"><h5>{{ __('Edit Faculty') }}: {{ $faculty->name }}</h5></div>
        <div class="card-body">
            <form method="POST" action="{{ route('faculties.update', $faculty) }}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code', $faculty->code) }}" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('dean_user_id') is-invalid @enderror" id="dean_user_id" name="dean_user_id">
                            ...
                            @foreach($users ?? [] as $user)
                                <option value="{{ $user->id }}" {{ old('dean_user_id', $faculty->dean_user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                            @endforeach
                        </select>
